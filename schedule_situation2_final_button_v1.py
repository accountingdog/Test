...
# project
project = pd.read_excel(r"C:\Users\ASUS\Desktop\situation2_scheduling\import_data.xlsx", sheet_name="project")
project.set_axis(project.iloc[:, 0], axis=0, inplace=True)  # 換掉列名稱
project = project.iloc[:, 1:]  # 移除第一欄
#print(project)
#print(project.loc['start_date', '專案一'])

# level
level = pd.read_excel(r"C:\Users\ASUS\Desktop\situation2_scheduling\import_data.xlsx", sheet_name="level")
level.set_axis(level.iloc[:, 0], axis=0, inplace=True)  # 換掉列名稱
level = level.iloc[:, 1:]  # 移除第一欄
#print(level)

# level_and_module
level_and_module = pd.read_excel(r"C:\Users\ASUS\Desktop\situation2_scheduling\import_data.xlsx", sheet_name="level_and_module")
level_and_module.set_axis(level_and_module.iloc[:, 0], axis=0, inplace=True)  # 換掉列名稱
level_and_module = level_and_module.iloc[:, 1:]  # 移除第一欄
#print(level_and_module)

# project_and_module
project_and_module = pd.read_excel(r"C:\Users\ASUS\Desktop\situation2_scheduling\import_data.xlsx", sheet_name="project_and_module")
project_and_module.set_axis(project_and_module.iloc[:, 0], axis=0, inplace=True)  # 換掉列名稱
    ...
    for p in range(len(permutations1)):  # p有240種
        ...
        for i in range(earlist_month, latest_month + 1): # 7-12
            ...
            for name in month_project[i]:  # 該月進行的專案
                ...
                # 先做PM 有餘力再做專長
                if project_and_module_remain.loc['PM', name] < 30:  # 可以做專長
                    ...
                    project_level_days.loc[name, 'L4'] += 30  # 加入做多少天df
                    project_and_module_finish_PM.loc['PM', name] += 30
                    project_and_module_remain.loc['PM', name] -= 30  # 扣掉已做
...
def download_excel_financial_data():  # sheet1,2,3
...
download_button1 = Button(window, text="Download financial_data", command=lambda:download_excel_financial_data())
